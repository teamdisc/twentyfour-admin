...
class ManageBooth extends Component {
  ...
  componentWillMount() {
    ...
    axios.get(`http://54.255.222.20:8080/exhibitions/${exhibitionId}/booths/${boothId}`)
      .then(response => {
        const {data} = response;
        this.setState({
          fetched: true,
  ...
  handleOnSubmit = (data) => {
    const {boothId, exhibitionId} = this.props.params;
    axios.post(`http://54.255.222.20:8080/exhibitions/${exhibitionId}/booths/${boothId}/update`, data)
      .then(response => {
